## R code for analysing blue confirmatory experiment
##
##


## low-sim = 1
## high-sim = 2
## free-viewing = 1
## fixed-viewing = 2
#########
# Setup #
#########

setwd('D:/Box Sync/Human Attention Lab Folders/Attention Project/Gavin/Log slopes')


folder = getwd()
# setwd(folder)
library(tidyverse)
library(stringr)
library(ggplot2)
library(eyelinkR)
library(ez)
library(broom)

number_ticks <- function(n) {function(limits) pretty(limits, n)}




every_nth <- function(x, nth, empty = TRUE, inverse = FALSE) 
{
  if (!inverse) {
    if(empty) {
      x[1:nth == 1] <- ""
      x
    } else {
      x[1:nth != 1]
    }
  } else {
    if(empty) {
      x[1:nth != 1] <- ""
      x
    } else {
      x[1:nth == 1]
    }
  }
}

#############
# Read Data #
#############
sim1 <- "Low-similarity"
sim2 <- "High-similarity"
eyeleave_val <- .20

all_df <- read_data("./Expt 2/expt2_data.csv", "confirm")
all_data <- all_df[[1]]
good_data <- clean_data(all_data, .9, eyeleave_val) # remove those with accuracy < 90% and with more than eyeleave_val% eye movements
good_subs <- unique(good_data$sub_id)
descrip <- get_desc(all_data)

accuracy <- descrip %>%
  filter(is.element(sub_id, good_subs)) %>%
  filter(condition == 2) 

mean(accuracy$accuracy)



#################
####  ANALYSES
#################

RT_free <- good_data %>%
  filter(condition==1) %>%
  group_by(sub_id, d_id, d_setsize, d_setsize_log, order, condition) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE))

RT_fixed <- good_data %>%
  filter(condition==2 & eye_leave==0) %>%
  group_by(sub_id, d_id, d_setsize, d_setsize_log, order, condition) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE))

RT_all <- rbind(RT_fixed, RT_free)
RT_all$d_id <- as.factor(RT_all$d_id)
RT_all$d_setsize <- as.factor(RT_all$d_setsize)
RT_all$order <- as.factor(RT_all$order)

# check for effects of viewing order
ezANOVA(RT_all %>% filter(d_id!=0),
        dv=meanrt,
        wid=sub_id,
        within=.(d_id, d_setsize),
        between=.(order))

## Collapse across order
RT_free <- good_data %>%
  filter(condition==1) %>%
  group_by(sub_id, d_id, d_setsize, d_setsize_log) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE))

RT_fixed <- good_data %>%
  filter(condition==2 & eye_leave==0) %>%
  group_by(sub_id, d_id, d_setsize, d_setsize_log) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE))

RT_fixed$d_id <- as.factor(RT_fixed$d_id)
RT_fixed$d_setsize <- as.factor(RT_fixed$d_setsize)

ezANOVA(RT_fixed %>% filter(d_setsize!=1),
        dv=meanrt,
        wid=sub_id,
        within=.(d_id, d_setsize))


##############################
## SLOPES ##
###########

target_only_data <- good_data %>%
  filter(d_id == 0)

target_1 <- target_only_data
target_1$d_id <- 1
target_2 <- target_only_data
target_2$d_id <- 2

slopes_data <- rbind(good_data %>% filter(d_id!=0), target_1, target_2)

## Condition: 1 == free, 2 == fixed


log_slopes <- slopes_data %>%
  # mutate(eyeleave_2 = if_else(condition==2 & saccade_latency>0, 1, 0)) %>%
  # mutate(eyeleave_1 = if_else(condition==1 & saccade_latency>0, 0, 1)) %>%
  filter((condition==2 & is.na(saccade_latency)) | (condition==1))%>%
  group_by(sub_id, d_id, condition, order, d_setsize_log) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE)) %>%
  do(slope = lm(meanrt ~ d_setsize_log, data=.)) %>%
  tidy(slope) %>%
  filter(term=="d_setsize_log") %>%
  select(-term, -std.error, -statistic, -p.value)

mean_log_slopes <- log_slopes %>%
  group_by(d_id, condition) %>%
  summarise(mean_log = mean(estimate))

linear_slopes <- slopes_data %>%
  mutate(eyeleave_2 = if_else(condition==2 & saccade_latency>0, 1, 0)) %>%
  # mutate(eyeleave_1 = if_else(condition==1 & saccade_latency>0, 0, 1)) %>%
  filter((condition==2 & is.na(saccade_latency)) | (condition==1))%>%
  group_by(sub_id, d_id, condition, d_setsize) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE)) %>%
  do(slope = lm(meanrt ~ d_setsize, data=.)) %>%
  tidy(slope) %>%
  filter(term=="d_setsize") %>%
  select(-term, -std.error, -statistic, -p.value)

mean_linear_slopes <- linear_slopes %>%
  group_by(d_id, condition) %>%
  summarise(mean_linear = mean(estimate))


slopes_data$half <- unlist(slopes_data$half)

slopes_half <- slopes_data %>%
  mutate(eyeleave = if_else(condition==2 & saccade_latency>0, 1, 0)) %>%
  filter(is.na(eyeleave) | eyeleave==0) %>%
  group_by(sub_id, d_id, half, block, d_setsize_log) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE)) %>%
  do(slope = lm(meanrt ~ d_setsize_log, data=.)) %>%
  tidy(slope) %>%
  filter(term=="d_setsize_log") %>%
  select(-term, -std.error, -statistic, -p.value)


t.test((slopes_half %>% filter(d_id==2, condition ==1, half==1))$estimate, 
       (slopes_half%>%filter(d_id==2, condition==1, half==2))$estimate, paired=TRUE)



t.test((slopes %>% filter(d_id==1, condition==1))$estimate, (slopes%>%filter(d_id==1, condition==2))$estimate, paired=TRUE)
t.test((slopes %>% filter(d_id==2, block==1))$estimate, (slopes%>%filter(d_id==2, block==2))$estimate, paired=TRUE)


slopes_wide <- log_slopes %>%
  spread(order, estimate)

log_slopes$order <- as.factor(log_slopes$order)
log_slopes$condition <- as.factor(log_slopes$condition)
log_slopes$block <- as.factor(log_slopes$block)
log_slopes$d_id <- as.factor(log_slopes$d_id)

ezANOVA(slopes_half %>%filter(d_id==1),
        dv=estimate,
        wid=sub_id,
        within=.(half),
        between=block)

ezANOVA(log_slopes,
        dv = estimate,
        wid=sub_id,
        within=condition,
        between=order)

ezANOVA(log_slopes,
        dv=estimate,
        wid=sub_id,
        within=.(condition, d_id),
        between=order)


slopes_mean <- slopes %>%
  filter(condition == 1) %>%
  group_by(d_id) %>%
  summarize(m_slope = mean(estimate))



        

orange_slopes <- get_slopes(orange%>%filter(), fixed_first_free)

t.test(fixed_first_slopes$orange_log_free, fixed_first_slopes$orange_log_fixed, paired=TRUE)


###############################
## FITS ##
##########
fits <- get_fits(RT_free, RT_fixed)
a <- fits[1]

##################
## AIC ##
#########


# since we have to include target-only into fits, create new variable for AIC/plotting
# then, add another row of target-only (since we need two, one for each lure type)
# then, assign that to the remaining lure type that doesn't have the target-only condition



## CHANGE THIS!!
plotdf <- RT_free

RT_plot <- plotdf %>%
  group_by(d_id, d_setsize, d_setsize_log) %>%
  summarise(mean_rt = mean(meanrt), sem = sd(meanrt)/sqrt(nrow(RT_fixed)/7)) %>%
  mutate(plot_id = ifelse((d_id==0 | d_id==1),1,2 ))

RT_plot <- rbind(RT_plot, RT_plot%>%filter(d_id==0))
RT_plot$plot_id[nrow(RT_plot)] <- 2
RT_plot$d_setsize <- as.numeric(as.character(RT_plot$d_setsize))

#### Get AIC


AIC(lm(mean_rt~(d_setsize_log), RT_plot %>%filter(plot_id==1)))
AIC(lm(mean_rt~(d_setsize_log), RT_plot %>%filter(plot_id==2)))

AIC(lm(mean_rt~(d_setsize), RT_plot %>%filter(plot_id==1)))
AIC(lm(mean_rt~(d_setsize), RT_plot %>%filter(plot_id==2)))

####  Get r-sq values 

orange_linear <- round((summary(lm(mean_rt~(d_setsize), RT_plot %>% filter(plot_id==1))))$r.squared, digits=3)
orange_log <- round((summary(lm(mean_rt~(d_setsize_log), RT_plot %>% filter(plot_id==1))))$r.squared, digits=4)
blue_linear <- round((summary(lm(mean_rt~(d_setsize), RT_plot %>% filter(plot_id==2))))$r.squared, digits=3)
blue_log <- round((summary(lm(mean_rt~(d_setsize_log), RT_plot %>% filter(plot_id==2))))$r.squared, digits=3)


#######################
## RT PLOT ##
#############


ggplot(RT_plot, aes(d_setsize, mean_rt,color=as.factor(plot_id))) + 
  stat_smooth(method="lm", formula=y~log(x), se=FALSE, linetype=1) +
  geom_point(data=RT_plot, aes(d_setsize, mean_rt, shape=as.factor(d_id), size=as.factor(d_id),color=as.factor(d_id))) +
  geom_errorbar(aes(ymin=mean_rt-sem, ymax=mean_rt+sem, color=as.factor(d_id)), width=.3) +
  scale_color_manual(values=c('#00CED1', '#FFA500', '#0000FF'), labels = c("Target-only", sim1, sim2)) +
  scale_shape_manual(values=c("0"=17, "1"=18, "2"=19), labels = c("Target-only", sim1, sim2)) +
  scale_size_manual(values=c(4,5,4), guide=FALSE) +
  xlab("\n Set size") +
  ylab("Reaction Time (ms) \n") +
  coord_cartesian(ylim=c(550,750))+
  # scale_y_continuous(breaks=number_ticks(6)) +
  scale_y_continuous(breaks=seq(550,750,10),
                     labels = every_nth(seq(550,750,10), 5, inverse=TRUE)) +
  theme_bw() + 
  theme(panel.border = element_rect(linetype = 'solid', colour = 'grey', size=1.2),
        text = element_text(size=14)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  theme(legend.position="none",
        axis.text=element_text(size=14)) 





