## R code for analysing red confirmatory experiment (1B)
##
##
...
all_df <- read_data("./Expt 1B/expt1b_data.csv", "confirm")
all_data <- all_df[[1]]
...
log_slopes <- slopes_data %>%
  ...
  group_by(sub_id, d_id, condition, d_setsize_log) %>%
  summarise(meanrt = mean(RT, na.rm=TRUE)) %>%
...
t.test(slopes$blue_log_free, slopes$blue_log_fixed, paired=TRUE)
