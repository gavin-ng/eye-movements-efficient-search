...
clean_data <- function(eyedata, accuracy_val, eyeleave_val){
  ...
  accuracy <- both_data %>% group_by(sub_id, condition) %>%
    summarise(accuracy =mean(hit)) %>%
    filter(accuracy >= accuracy_val) %>%
    distinct()
  
  leave_bad <- (descrip %>%
                  filter(condition==2 & eye_leave> eyeleave_val))$sub_id
  
  
  
  good_subs_both <- both_data %>%
    filter(is.element(sub_id, accuracy$sub_id)  & hit ==1 & !is.element(sub_id, leave_bad)) %>%
    group_by(sub_id) %>%
  ...
  good_data$eye_leave <- as.numeric(good_data$eye_leave)
